...
class Serie {
    ...
    public function readActive() {
        ...
        FROM serie ser JOIN serie_genre sg ON ser.SerieID = sg.SerieID JOIN genre g ON sg.GenreID = g.GenreID WHERE ser.Actief = 1 GROUP BY ser.SerieID, ser.IMDBLink, ser.SerieTitel ORDER BY ser.SerieID;";
        $stmt = $this->conn->prepare($query);
        $stmt->execute();
        return $stmt;
    }
    ...
    public function search($keyword) {
        $query = "SELECT * FROM " . $this->table_name . " WHERE Actief = 1 and SerieTitel LIKE ?";
